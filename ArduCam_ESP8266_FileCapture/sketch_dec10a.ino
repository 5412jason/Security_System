...
String deviceId = "v0995B81803BB122";
...
// if the video is chopped or sedistored, try using a lower value for the buffer
...
static IPAddress ip(192, 168, 137, 212); // static IP used for browser access: http://IPaddress
static IPAddress gateway(141, 215, 217, 251);//217 251
...
void setup() {
  ...
  server.on("/data", HTTP_GET, serverData);
...
String getPage(){
  ...
  page += "<li>Humidity : ";
  page += h;
  page += "<h3>Smoke Sensor</h3>";
  page += "<ul><li>Smoke Level: ";
  page += smokeValue;
  page += "<h3>Motion Detected</h3>";
  page += "<ul><li>Yes or No: ";
  page += motionValue;
  page += "</body></html>";
  return page;
}
...
void loop() {
  ...
  h = dht.readHumidity();
  smokeValue=digitalRead(D3);
  if(digitalRead(8)==HIGH){
    //motion issue in house
    motionValue="Yes";
  }
